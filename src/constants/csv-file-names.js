const CSVFileNames = {
  COUNTRY_DISTRIBUTION: 'ec-tel-percentage-authors-per-country.csv',
  FIELD_DISTRIBUTION: 'ec-tel-percentage-field-distribution.csv',
  GENDER_DISTRIBUTION_PER_FIELD: 'ec-tel-percentage-gender-distribution-per-field.csv',
  GENDER_DISTRIBUTION: 'ec-tel-percentage-gender-distribution.csv',
  GENDER_DIVERSITY_BY_PAPER: 'ec-tel-percentage-author-gender-diversity-by-paper.csv',
  UNIVERSITY_DISTRIBUTION: 'ec-tel-num-unique-authors-per-institution.csv',
  WOMEN_1ST_AUTHORS_DISTRIBUTION: 'ec-tel-percentage-women-1st-author-distribution.csv',
};
export default CSVFileNames;
